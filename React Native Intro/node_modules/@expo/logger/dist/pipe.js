...
function pipe(stream, loggerFn, transformer) {
    const multilineLogger = createMultilineLogger(loggerFn, transformer);
    stream.on('data', multilineLogger);
}
exports.pipe = pipe;
function pipeSpawnOutput(logger, { stdout, stderr } = {}, { printAsStdoutOnly = false } = {}) {
    if (stdout) {
        const stdoutLogger = logger.child({ source: 'stdout' });
        pipe(stdout, (line) => stdoutLogger.info(line));
    }
    if (stderr) {
        const stderrLogger = logger.child({ source: printAsStdoutOnly ? 'stdout' : 'stderr' });
        pipe(stderr, (line) => stderrLogger.info(line));
    }
}
