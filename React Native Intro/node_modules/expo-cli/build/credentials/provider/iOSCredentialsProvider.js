...
class iOSCredentialsProvider {
  ...
  async hasRemoteAsync() {
    const distCert = await this.ctx.ios.getDistCert(this.projectFullName, this.options.bundleIdentifier);
    const provisioningProfile = await this.ctx.ios.getProvisioningProfile(this.projectFullName, this.options.bundleIdentifier);
  ...
  async getRemoteAsync() {
    await (0, _route().runCredentialsManager)(this.ctx, new (_SetupIosBuildCredentials().SetupIosBuildCredentials)({
