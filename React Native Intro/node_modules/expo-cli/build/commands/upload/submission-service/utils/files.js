...
async function createTemporaryDirectoryForExtractionAsync() {
  // Since we may need to rename the destination path,
  // add everything to a folder which can be nuked to ensure we don't accidentally use an old build with the same name.
  const destinationFolder = (0, _path().join)(_os().default.tmpdir(), 'expo-submission-service');

  if (await _fsExtra().default.pathExists(destinationFolder)) {
    await _fsExtra().default.remove(destinationFolder);
  }

  await _fsExtra().default.ensureDir(destinationFolder);
